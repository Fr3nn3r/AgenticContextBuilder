...
# Re-export for backwards compatibility (moved to utils.number_parsing)
...
class AggregatedLineItem(BaseModel):
    ...
    Numeric fields (quantity, unit_price, total_price) accept European number
    formats and strings with embedded currency/units, e.g.:
    - "249,77" -> 249.77
    - "SFr. 358,00" -> 358.00
    - "0,2 h" -> 0.2
    """
    ...
    @classmethod
    def parse_numeric_field(cls, v):
        """Parse numeric fields that may contain European formats or units."""
        return parse_european_number(v)
