...
class LabelsService:
    ...
    def save_classification_label(
        ...
        if not doc_type_correct:
            # Log override decision when user marks classification as incorrect
            ...
            # Log approval decision when user confirms classification is correct
            self._log_classification_approval(
                doc_id=doc_id,
                claim_id=label_data.get("claim_id", ""),
    ...
    def _log_classification_approval(
        self,
        doc_id: str,
        claim_id: str,
        doc_type: str,
        ...
        """Log classification approval decision to the compliance ledger.

        Records when a human confirms that the AI classification is correct.

        Args:
            doc_id: Document identifier
            claim_id: Claim identifier
            doc_type: The document type that was approved
            notes: Review notes
        """
        try:
            rationale = DecisionRationale(
                summary=f"Classification approved: {doc_type}",
                confidence=1.0,  # Human approvals are authoritative
                notes=notes if notes else None,
            )
            ...
            record = DecisionRecord(
                ...
                actor_type="human",
                actor_id="reviewer",  # Could be enhanced to capture actual reviewer
                metadata={"classification_approved": True, "doc_type": doc_type},
            )

            self.decision_ledger.append(record)
            logger.debug(f"Logged classification approval for doc_id={doc_id}")

        except Exception as e:
            logger.warning(f"Failed to log classification approval: {e}")
