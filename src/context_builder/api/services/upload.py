...
class UploadService:
    ...
    def __init__(
        ...
        Args:
            ...
            ledger_dir: Directory for compliance logging (e.g., output/logs)
        """
        self.staging_dir = staging_dir
        self.claims_dir = claims_dir
        self.staging_dir.mkdir(parents=True, exist_ok=True)
        # Initialize decision ledger for compliance logging
        self.decision_ledger: Optional[DecisionLedger] = None
        if ledger_dir:
    ...
    async def add_document(self, claim_id: str, file: UploadFile) -> PendingDocument:
        ...
        # Log document upload to compliance ledger
        self._log_document_upload(claim_id, doc)
    ...
    def _log_document_upload(self, claim_id: str, doc: PendingDocument) -> None:
        """Log document upload to the compliance ledger.

        Records when a user uploads a document to the staging area.

        Args:
            claim_id: Claim identifier the document was uploaded to
            doc: The pending document that was uploaded
        """
        ...
        try:
            rationale = DecisionRationale(
                summary=f"Document uploaded: {doc.original_filename}",
                confidence=1.0,
                notes=f"File type: {doc.content_type}, Size: {doc.file_size} bytes",
            )
            ...
            logger.debug(f"Logged document upload for doc_id={doc.doc_id}")

        except Exception as e:
            logger.warning(f"Failed to log document upload: {e}")
